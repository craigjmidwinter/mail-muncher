...
type Engine struct {
	rules []compiledRule
	files *DomainFiles
	// domainFilePaths is every externally-owned domain list the compiled rules
...
func NewEngine(rules []config.Rule, opts ...Option) (*Engine, error) {
	...
	e.domainFilePaths = o.domainFiles
	return e, nil
}

// BeginCycle starts a new pipeline cycle: every externally-owned domain list is
...
// Preload reads every externally-owned domain list the rules reference, now,
// instead of lazily on first use. The pipeline calls it straight after
// BeginCycle so that Degraded is authoritative before the first message is
// fetched — which is what makes "store nothing when a list is unreadable"
// implementable.
func (e *Engine) Preload() {
	if e == nil {
		return
	}
	e.files.Preload(e.domainFilePaths)
}

// Degraded reports the externally-owned domain lists that could not be read in
// full during this cycle.
//
...
// reference, deduplicated, in first-use order.
func (e *Engine) DomainFilePaths() []string {
	if e == nil {
		return nil
	}
	return append([]string(nil), e.domainFilePaths...)
}
...
// DomainFiles exposes the engine's per-cycle domain-list cache, for callers
// that want to share it with something they compiled themselves.
func (e *Engine) DomainFiles() *DomainFiles {
	if e == nil {
		return nil
	}
	return e.files
